...
def main():
    ...
    if hackedMessage != None:
        # The plaintext is displayed on the screen. For the convenience of the user,
        # we copy the text of the code to the clipboard
        print(hackedMessage)
    else:
...
def hackAffine(message):
    ...
    for key in range(len(AffineCiphers.affineCipher.SYMBOLS) ** 2):
        keyA = AffineCiphers.affineCipher.getKeyParts(key)[0]
        if AffineCiphers.cryptomath.gcd(keyA, len(AffineCiphers.affineCipher.SYMBOLS)) != 1:
            continue

        decryptedText = AffineCiphers.affineCipher.decryptMessage(key, message)
        if not SILENT_MODE:
